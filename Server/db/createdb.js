...
async function createUsersTable(err) {
	if(err) throw err;
	var sql = "CREATE TABLE IF NOT EXISTS users(UserID int NOT NULL AUTO_INCREMENT, " +
...
async function createLeagueTable(err) {
	if(err) throw err;
	var sql = "CREATE TABLE IF NOT EXISTS  League(LeagueID int NOT NULL AUTO_INCREMENT,"+
...
async function createTeamTable(err) {
	if(err) throw err;
	var sql = "CREATE TABLE IF NOT EXISTS Team(TeamID int NOT NULL AUTO_INCREMENT,"+
...
async function createPlaysForTable(err) {
	if(err) throw err;
	var sql = "CREATE TABLE IF NOT EXISTS PlaysFor(TeamID int, UserID int, " +
...
async function createFixturesTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS FIXTURE(FixtureID int NOT NULL AUTO_INCREMENT" +
...
async function createSeasonsTable(err) {
	if(err) throw err;
	var sql = "CREATE TABLE IF NOT EXISTS Season(SeasonID int NOT NULL AUTO_INCREMENT," +
...
async function createFootballResultsTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS FootballResults(FixtureID int, HomeGoalsScoredHT int,"+
...
async function createRugbyResultsTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS RugbyResults(FixtureID int, HomePointsScoredHT int,"+
...
async function createAmericanFootballResultsTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS AmericanFootballResults(FixtureID int, HomePointsScoredQ1 int,"+
...
async function createBasketballResultsTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS BasketballResults(FixtureID int, HomePointsScoredQ1 int,"+
  " AwayPointsScoredQ1 int, HomePointsScoredHT int, AwayPointsScoredHT int,"+
  " HomePointsScoredQ3 int, AwayPointsScoredQ3 int, HomePointsScoredFT int, "+
  "AwayPointsScoredFT int, MatchDescription VARCHAR(300), PRIMARY KEY (FixtureID), "+
  "FOREIGN KEY (FixtureID) REFERENCES Fixture(FixtureID));";
...
async function createTennisResultsTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS TennisResults(FixtureID int, HomePointsScoredS1 int,"+
...
async function createCricketResultsTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS CricketResults(FixtureID int, HomeRunsI1 int,"+
...
async function createVolleyballResultsTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS VolleyballResults(FixtureID int, HomePointsScoredG1 int,"+
...
async function createHockeyResultsTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS HockeyResults(FixtureID int, HomePointsScoredHT int,"+
  " AwayPointsScoredHT int,HomePointsScoredFT int, AwayPointsScoredFT int, " +
  "MatchDescription VARCHAR(300), PRIMARY KEY (FixtureID), "+
  "FOREIGN KEY (FixtureID) REFERENCES Fixture(FixtureID));";
  await db.query(sql, function(err, result){
...
async function createTableTennisResultsTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS TableTennisResults(FixtureID int, HomePointsScoredG1 int,"+
  " AwayPointsScoredG1 int, HomePointsScoredG2 int, AwayPointsScoredG2 int,"+
  " HomePointsScoredG3 int, AwayPointsScoredG3 int, HomePointsScoredG4 int, "+
  "AwayPointsScoredG4 int, HomePointsScoredG5 int, AwayPointsScoredG5 int, " +
  "MatchDescription VARCHAR(300), PRIMARY KEY (FixtureID),"+
...
async function createFootballRankingsTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS FootballRankings(SeasonID int, TeamID int,Played int," +
...
async function createAmericanFootballRankingsTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS AmericanFootballRankings(SeasonID int, TeamID int, Played int," +
...
async function createRugbyRankingsTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS RugbyRankings(SeasonID int, TeamID int, Played int," +
  " Wins int, Draws int, Losses int, PointsScored int, PointsConceded int, Points int," +
  "PRIMARY KEY (SeasonID, TeamID)," +
  " FOREIGN KEY (SeasonID) REFERENCES season(SeasonID), " +
  " FOREIGN KEY (TeamID) REFERENCES team(TeamID));";
...
async function createBasketballRankingsTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS BasketballRankings(SeasonID int, TeamID int, Played int," +
  " Wins int, Draws int, Losses int, PointsScored int, PointsConceded int, Points int," +
  "PRIMARY KEY (SeasonID, TeamID)," +
  " FOREIGN KEY (SeasonID) REFERENCES season(SeasonID), " +
  " FOREIGN KEY (TeamID) REFERENCES team(TeamID));";
...
async function createTennisRankingsTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS TennisRankings(SeasonID int, TeamID int, Played int," +
...
async function createTableTennisRankingsTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS TableTennisRankings(SeasonID int, TeamID int, Played int," +
  " Wins int, Draws int, Losses int, SetsFor int, SetsAgainst int, Points int," +
  "PRIMARY KEY (SeasonID, TeamID)," +
  " FOREIGN KEY (SeasonID) REFERENCES season(SeasonID), " +
  " FOREIGN KEY (TeamID) REFERENCES team(TeamID));";
...
async function createHockeyRankingsTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS HockeyRankings(SeasonID int, TeamID int, Played int," +
  " Wins int, Draws int, Losses int, GoalsScored int, GoalsConceded int, Points int," +
  "PRIMARY KEY (SeasonID, TeamID)," +
  " FOREIGN KEY (SeasonID) REFERENCES season(SeasonID), " +
  " FOREIGN KEY (TeamID) REFERENCES team(TeamID));";
...
async function createCricketRankingsTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS CricketRankings(SeasonID int, TeamID int, Played int," +
...
async function createVolleyballRankingsTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS VolleyballRankings(SeasonID int, TeamID int, Played int," +
...
async function createPasswordResetTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS passwordReset(UserID int, resetToken VARCHAR(40), "+
...
async function createTeamAnnouncementTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS teamAnnouncement(AnnouncementID int NOT NULL AUTO_INCREMENT,"+
...
async function createLeagueAnnouncementTable(err) {
  if(err) throw err;
  var sql = "CREATE TABLE IF NOT EXISTS leagueAnnouncement(LeagueAnnouncementID int NOT NULL AUTO_INCREMENT,"+
...
async function createDB() {
  ...
  await createLeagueTable();
  await createTeamTable();
  await createPlaysForTable();
  await createSeasonsTable();
  await createFixturesTable();
  await createPasswordResetTable();
  await createTeamAnnouncementTable();
  await createLeagueAnnouncementTable();

  // ------  create Results tables  ------

  await createFootballResultsTable();
  await createRugbyResultsTable();
  await createAmericanFootballResultsTable();
  await createBasketballResultsTable();
  await createTennisResultsTable();
  await createCricketResultsTable();
  await createVolleyballResultsTable();
  await createHockeyResultsTable();
  await createTableTennisResultsTable();

  // ------  create Rankings tables  ------

  await createFootballRankingsTable();
  await createAmericanFootballRankingsTable();
  await createRugbyRankingsTable();
  await createBasketballRankingsTable();
  await createTennisRankingsTable();
  await createTableTennisRankingsTable();
  await createHockeyRankingsTable();
  await createCricketRankingsTable();
  await createVolleyballRankingsTable();
}
